...
{
    public class NNTeacherTest
    {
        private readonly Random random = new Random();

        private readonly NNCellBrainTest brain;

        private readonly double[][,] m;
        private readonly double[][,] v;
        private int t = 0;

        public NNTeacherTest(NNCellBrainTest brain)
        {
            this.brain = brain;
        ...
        public void UseExpToLearn(bool IsErrorMove, double[][] LastMovesInputs, int[] LastMovesDecidedActionsNum, bool[] ErrorMoves)
        {
            if (IsErrorMove)
            {
                ...
                if (LastMovesDecidedActionsNum[0] != -1)
                {
                    ...
                    LearnErrorFromExp(inputs, errorTarget, LastMovesInputs[0]);
                }
            }
            else
            {
